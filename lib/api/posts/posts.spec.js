...
describe('Test case: Posts', function() {
	...
	describe("Publishing posts", function() {
		"use strict";

		it("Should publish a post as a draft", function(done) {
			client1.emit('post::publish', {id: savedPost._id, isDraft: true});

			client1.on('post::updated', function(msg) {
				expect(msg).to.have.property('_id');
				expect(msg._id).to.equal(savedPost._id);
				expect(msg.attrs.status).to.equal("draft");
				done();
			});
		});

		it("Should publish a post", function(done) {
			client1.emit('post::publish', {id: savedPost._id, isDraft: false});

			client1.on('post::updated', function(msg) {
				expect(msg).to.have.property('_id');
				expect(msg._id).to.equal(savedPost._id);
				expect(msg.attrs).to.have.property('publish_date');
				expect(msg.attrs.status).to.equal("published");
				done();
			});
		});

		it('should have status change to "edited" if updates are made after publishing', function(done) {
			savedPost.text = "blah";
			...
			client1.on('post::updated', function(msg) {
				...
				expect(msg).to.have.property('_id');
				expect(msg.attrs.status).to.equal("edited");
				done();
