	...
	describe("Publishing posts", function() {
		...
		it("Should publish a post as a draft", function(done) {
			...
			client1.on('post::updated', function(msg) {
				expect(msg).to.have.property('_id');
				expect(msg._id).to.equal(savedPost._id);
				expect(msg.attrs.is_published).to.equal(true);
				expect(msg.attrs.is_draft).to.equal(true);
				done();
			});
		});
		...
		it("Should publish a post", function(done) {
			...
			client1.on('post::updated', function(msg) {
				...
				expect(msg._id).to.equal(savedPost._id);
				expect(msg.attrs.is_published).to.equal(true);
				expect(msg.attrs.is_draft).to.equal(false);
				done();
			});
		});

		it('should revert to draft status if updates are made after publishing', function(done) {
			savedPost.text = "blah";
